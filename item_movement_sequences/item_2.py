...
def get_sequence(gr):
    return [
        ...
        (3, "Move to Position (Approach)", lambda: move_to("position_approach_item_2", 4)),
        (4, "Move to Position (Grasp)", lambda: move_to("position_grasp_item_2", 3, "movel")),
        (5, "Close Gripper", lambda: gr.execute_gripper_script("close_gripper.script", call_function="gripper_close")),
        (6, "Move to Default Position via Approach Position", lambda: move_to("position_approach_item_2", 5, "movec", target="default_position")),
